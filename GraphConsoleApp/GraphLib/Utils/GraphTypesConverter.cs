...
internal static class GraphTypesConverter
{
  ...
  public static UndirectedGraph<string, UndirectedEdge<string>> IntToString(UndirectedGraph<int, UndirectedEdge<int>> intGraph)
  {
    ...
    foreach (var UndirectedEdge in intGraph.Edges)
    {
      ...
      stringGraph.AddVerticesAndEdge(new UndirectedEdge<string>(ModularProductAlgorithm.MinVertex(source, target), ModularProductAlgorithm.MaxVertex(source, target)));
    }
